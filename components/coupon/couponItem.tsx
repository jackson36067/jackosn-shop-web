...
const CouponContent = (props: {
  ...
  return (
    <div className="mt-10 px-5">
      {props.userCouponItems[0] && (
        <div>
          <div className="font-bold text-xl">平台卷</div>
          <div className="mt-5">
            {props.userCouponItems[0].memberCouponList.length > 0 &&
              (
                props.userCouponItems[0]
                  .memberCouponList as memberCouponItemVOList[]
              ).map((item: memberCouponItemVOList) => {
                return (
                  <div key={item.id} className="flex-1 flex items-center">
                    {props.deleteState && (
                      <div className="flex-1/10">
                        <Checkbox
                          checked={SelectedCouponId.includes(item.id)}
                          onCheckedChange={(checked) =>
                            handleCheckCoupon([item.id], checked === true)
                          }
                    ...
                    <div className="flex flex-9/10 items-center bg-[#ffece5]/90 rounded-lg">
                      <div className="flex-1/4 flex flex-col items-center justify-center border-r-[1px] border-red-500 border-dashed py-4 text-red-600">
                        ...
                        <div className="text-[0.8rem]">满{item.min}可用</div>
                      </div>
                      <div className="flex-3/4 flex justify-between items-center px-2">
                        <div className="text-[0.8rem] text-red-600">
                          <p className="text-black text-[1rem]">{item.title}</p>
                          <p>{item.expireTime}到期</p>
                          <p>限该店铺内商品可用</p>
                        </div>
      ...
      {props.userCouponItems[1] && (
        <div className="mt-10 mb-5">
          <div className="font-bold text-xl">店铺卷</div>
          <div className="mt-5">
            {props.userCouponItems[1].memberCouponList.length > 0 &&
              (
                props.userCouponItems[1].memberCouponList as UserCouponItem[]
              ).map((item: UserCouponItem) => {
                return (
                          ...
                          <div className="flex items-center pr-2">
                            <Checkbox
                              checked={item.memberCouponItemVOList
                                .map((item) => item.id)
                                .every((item) =>
                                  SelectedCouponId.includes(item)
                                )}
                              onCheckedChange={(checked) =>
                                handleCheckCoupon(
                                  item.memberCouponItemVOList.map(
                                    (item) => item.id
                                  ),
                                  checked === true
                                )
                          ...
                          return (
                            ...
                            >
                              {props.deleteState && (
                                <div className="flex-1/10">
                                  <Checkbox
                                    checked={SelectedCouponId.includes(
                                      couponItem.id
                                    )}
                                    onCheckedChange={(checked) =>
                                      handleCheckCoupon(
